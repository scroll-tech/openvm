...
use super::{columns::FullIndexedOutputPageCols, FullIndexedOutputPageAir};

impl<F: PrimeField64> SubAirBridge<F> for FullIndexedOutputPageAir {
    fn receives(&self, col_indices: FullIndexedOutputPageCols<usize>) -> Vec<Interaction<F>> {
    ...
    fn sends(&self, col_indices: FullIndexedOutputPageCols<usize>) -> Vec<Interaction<F>> {
        let mut interactions = vec![];
...
impl<F: PrimeField64> AirBridge<F> for FullIndexedOutputPageAir {
    fn receives(&self) -> Vec<Interaction<F>> {
        let num_cols = self.air_width();
        let all_cols = (0..num_cols).collect::<Vec<usize>>();

        let cols_to_receive = FullIndexedOutputPageCols::<usize>::from_slice(
            &all_cols,
            self.idx_len,
            self.data_len,
