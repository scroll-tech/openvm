    ...
    Com<SC>: Into<[Val<SC>; COMMITMENT_LEN]>,
{
    ...
    //     #[allow(clippy::too_many_arguments)]
    //     pub fn generate_get_top_p_cum<P: PageProvider + ProverTraceDataProvider<SC>>(
    //         provider: &mut P,
    //         engine: &impl StarkEngine<SC>,
    //         trace_builder: &mut TraceCommitmentBuilder<SC>,
    //         page_controllers: &mut PageControllers<SC, COMMITMENT_LEN>,
    ...
    //         remaining: &Vec<u32>,
    //         buf: &[DataFrameRow<u32>],
    //         indexed_table: &mut Vec<DataFrameRow<u32>>,
    //         m: usize,
    //     ) -> (GetTopPCum<SC>, Vec<u32>) {
    //         let k = page_controllers.params.k;
    //         if m == 0 {
    //             (GetTopPCum::Empty(), vec![])
    //         } else {
    //             let (head, init_remaining_commit) = Self::generate_get_top_p_cum(
    //                 provider,
    //                 engine,
    //                 trace_builder,
    //                 page_controllers,
    ...
    //                 .generate_output_pages(&init_remaining, &input_page);
    //             let input_page_pdata = provider.load_pdata_by_commitment(&buf[tail_idx].commitment);
    //             let init_remaining_pdata = provider.load_pdata_by_commitment(remaining);
    //             provider.remove_page_by_commitment(&init_remaining_commit);
    //             provider.remove_page_by_commitment(&buf[tail_idx].commitment);
    //             provider.remove_pdata_by_commitment(&buf[tail_idx].commitment);
    //             provider.remove_pdata_by_commitment(&init_remaining_commit);
    ...
    //                 top_p_pdata: None,
    //             };
    //             let pdata = page_controllers.get_top_p.load_pages(
    //                 &init_remaining,
    //                 &input_page,
    //                 &final_remaining_page,
    //                 &top_p_page,
    ...
    //                 &mut trace_builder.committer,
    //             );
    //             let top_p_commit = pdata_to_commit_u32(&pdata.top_p_pdata.clone().unwrap());
    //             let remaining = pdata_to_commit_u32(&pdata.final_remaining_pdata.clone().unwrap());

    //             provider.add_pdata_with_commitment(
    //                 &remaining,
    //                 pdata.final_remaining_pdata.clone().unwrap(),
    //             );
    //             provider.add_page_with_commitment(&remaining, &final_remaining_page);
    //             provider.add_pdata_with_commitment(&top_p_commit, pdata.top_p_pdata.clone().unwrap());
    //             provider.add_page_with_commitment(&top_p_commit, &top_p_page);
