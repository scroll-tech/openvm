...
fn build_test_program<C: Config>(builder: &mut Builder<C>) {
    ...
    let prod_spec_evals: Array<C, Ext<C::F, C::EF>> =
        builder.dyn_array(prod_spec_eval_u32s.len() / EXT_DEG);
    ...
    let logup_spec_evals: Array<C, Ext<C::F, C::EF>> =
        builder.dyn_array(logup_spec_eval_u32s.len() / EXT_DEG);
