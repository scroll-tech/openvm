...
/// The auxiliary columns for a hint read operation.
/// Stores the previous timestamp to enable permutation checking through the hint bus.
#[repr(C)]
#[derive(Clone, Copy, Debug, AlignedBorrow)]
pub struct HintReadAuxCols<T> {
    /// The previous timestamp when this hint location was accessed
    pub prev_timestamp: T,
}

impl<F: PrimeField32> HintReadAuxCols<F> {
    pub fn new(prev_timestamp: F) -> Self {
        Self { prev_timestamp }
    }
...
/// The auxiliary columns for a hint write operation.
/// Stores the previous timestamp and previous value to enable permutation checking through the hint bus.
#[repr(C)]
#[derive(Clone, Copy, Debug, AlignedBorrow)]
pub struct HintWriteAuxCols<T> {
    /// The previous timestamp when this hint location was accessed
    pub prev_timestamp: T,
    /// The previous value at this hint location
    pub prev_value: T,
}

impl<F: PrimeField32> HintWriteAuxCols<F> {
    pub fn new(prev_timestamp: F, prev_value: F) -> Self {
        Self {
            prev_timestamp,
            prev_value,
        }
    ...
    #[inline(always)]
    pub fn set_prev(&mut self, timestamp: F, value: F) {
        self.prev_timestamp = timestamp;
